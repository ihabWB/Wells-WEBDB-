...
// - Upserts on unique (well_id, year, month)
// - Abstraction auto-calculated from meters if left blank; if user types a value, it is saved as manual
// - Validates numbers and meter progression
(function(){
  'use strict';

  // Reuse global Supabase client if present, else create from window.APP_CONFIG
  let sb = window.$sb;
  if (!sb && window.APP_CONFIG && window.supabase) {
  ...
  if (!sb) {
    console.warn('Supabase client not found. Ensure APP_CONFIG and supabase-js are loaded.');
    return;
  }

  const $ = (id) => document.getElementById(id);

  // Optional: hook Single vs Batch toggle if present
  ...
  async function init(){
    await loadWells();
    if (rbAddRow) rbAddRow.addEventListener('click', ()=> addRow());
    if (rbDuplicateLast) rbDuplicateLast.addEventListener('click', duplicateLast);
    if (rbClear) rbClear.addEventListener('click', ()=> { rows = []; renderBatch(); setBatchStatus('Cleared.'); });
    if (rbSaveAll) rbSaveAll.addEventListener('click', saveAllBatch);

    // Start with one row
    addRow();

    // If wells list changes elsewhere in app, refresh selects
  ...
  async function loadWells(){
    try{
      const { data, error } = await sb.from('wells').select('well_id, well_code').order('well_code',{ascending:true}).limit(5000);
  ...
  function addRow(seed){
    ...
    rows.push({
      ...
      last: seed?.last ?? '',
      curr: seed?.curr ?? '',
      abs: seed?.abs ?? '',       // if '', we will send NULL and DB trigger will fill
  ...
  async function saveAllBatch(){
    ...
    try{
      // Upsert by (well_id, year, month) — year/month are generated on the server
